...
overlay_histo <- ggplot(histo_df, aes(x=values, fill=group)) + geom_density(position="identity", alpha=0.3) + 
        theme(legend.position = "none") + labs(x="Mean Score", y="") + 
        scale_fill_manual(values=c("#00bfc4", "#619cff", "#b79f00", "#f564e3", "#00ba38", "#f8766d"))

overlay_histo

```

\textbf{We see that student's whose parents achieved beyond a bachelor's degree are more likely to score beyond $80\%$. Furthermore, student's whose parents did not attend college are more likely to score below $60\%$. Otherwise, it appears to be difficult to differentiate between different levels of parental education.}

### Next, we will look at how race affects subject specific exam scores. 
